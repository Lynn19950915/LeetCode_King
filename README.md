### 專案簡介

LeetCode 刷題日記，記錄我在解題的工具、思路，以及討論區的閱讀內容。

這份筆記會以解法整理為核心，鼓勵大家消化後寫出自己的程式碼！

- 備註：解法的排序會由繁而簡，越靠後的空間、時間複雜度會越低。


<p align="center">
    <img src="https://upload.cc/i1/2022/07/08/5QiPct.png" style="padding-left: 25%; width: 50%">
</p>

### 搜尋方式

<table style="table-layout: fixed">
    <thead>
        <td align="center" width="500vmax"> 方法 </td>
        <td align="center" width="250vmax"> 題數 </td>
        <td align="center" width="250vmax"> 入口 </td>
    </thead>
    <tr>
        <td align="center"> Easy </td>
        <td align="center"> 2 </td>
        <td align="center"> [點我](https://github.com/Lynn19950915/LeetCode_King/tree/main/Easy%20(2)) </td>
    </tr>
    <tr>
        <td align="center"> Tags </td>
        <td align="center"> - </td>
        <td align="center"> [點我](https://github.com/Lynn19950915/LeetCode_King/tree/main/Tags) </td>
    </tr>
</table>
